...
def get_basic_feat(data, EMBEDDING_DIM, stop_words, word2vec):
    ...
    data['common_words'] = data.apply(
        lambda x: len(set(x['question1'].lower().split('|')).intersection(set(x['question2'].lower().split('|')))),
        axis=1)
...
def model_simscore(pred_file, data):
    df = pd.read_csv(pred_file, sep="\t", encoding="utf-8", names=["question1", "question2", "label", "simscore"])
