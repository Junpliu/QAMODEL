...
class ARCIIT(object):
    ...
    def arcii(self, embed_seq1, embed_seq2, seq_len1, seq_len2, max_seq_len1, max_seq_len2, name, reuse):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            # Layer-1: conv1d
            x_con1d = tf.layers.conv1d(embed_seq1, self.num_filters, k1, 1, 'same', name="1/con1d1", activation=tf.nn.relu)
            y_con1d = tf.layers.conv1d(embed_seq2, self.num_filters, k1, 1, 'same', name="1/con1d2", activation=tf.nn.relu)
            ...
            # Layer-2: pooling2d
            pool2d = tf.layers.max_pooling2d(xy_con1d, (2, 2), 2, padding="same", name="2/pool2d_1")

            # Layer-3: conv2d
            con2d = tf.layers.conv2d(pool2d, self.num_filters, (2, 2), padding="same", name="3/con2d_1", activation=tf.nn.relu)

            # Layer-4-6: pooling2d, conv2d, pooling2d
            pool2d_2 = tf.layers.max_pooling2d(con2d, (2, 2), 2, padding="same", name="4/pool2d_2")
    ...
    def build_graph(self):
        with tf.variable_scope("net"):
            ...
            word_sim_12 = self.arcii(word_embed1, word_embed2, self.word_len1, self.word_len2,
                                     self.max_word_len1, self.max_word_len2, "word", tf.AUTO_REUSE)
            char_sim_12 = self.arcii(char_embed1, char_embed2, self.char_len1, self.char_len2,
                                     self.max_char_len1, self.max_char_len2, "char", tf.AUTO_REUSE)

            self.s_12 = word_sim_12 * 0.5 + char_sim_12 * 0.5
            ...
            if self.mode != "infer":
                ...
                word_sim_13 = self.arcii(word_embed1, word_embed3, self.word_len1, self.word_len3,
                                         self.max_word_len1, self.max_word_len2, "word", tf.AUTO_REUSE)
                char_sim_13 = self.arcii(char_embed1, char_embed3, self.char_len1, self.char_len3,
                                         self.max_char_len1, self.max_char_len2, "char", tf.AUTO_REUSE)
        ...
        if self.mode != "infer":
            with tf.variable_scope("loss"):
                self.losses = tf.reduce_mean(
                    tf.maximum(0.0, self.margin - self.s_12 + self.s_13), name="max_margin_loss")
